...
- When `VPN_USERNAME` / `VPN_PASSWORD` are set, automation handles both account-picker and credential forms via CDP. Autofill is idempotent: if Microsoft leaves the username or password step visible after submission, the same value is not typed and submitted again on each poll.
