...
| **`jumphost` binary** | Single Rust binary built from the repo root (`target/release/jumphost`). One process owns everything: cookie management, openconnect supervision, the routing proxy, and the PAC HTTP server. Subcommands include `run`, `refresh_token`, `validate-cookie`, `generate-pac`, `authenticate`, `deauthenticate`, `doctor`, `logs`, and `test-tunnel`. `refresh_token` forces a fresh cookie fetch using an existing complete credential source and never prompts for credentials. |
...
sequenceDiagram
  ...
  JH->>Chromium: launch with persistent profile and no proxy
...
- Validates the cookie file synchronously at startup (reqwest GET of `<vpn_url>/vdesk/vpn/index.php3?outform=xml`, proxy discovery and redirects disabled — 2xx = valid, 3xx/404 = invalid, network error = unknown).
- If invalid or missing, launches Chromium via `chromiumoxide` with `--no-proxy-server` for a refresh and writes the refreshed cookie to the cookie file (mode 600). The portal and all SSO/MFA redirects therefore use direct host networking and do not consult the PAC, routing proxy, or `[domains]` rules. In headless Microsoft Authenticator number-match flows, `src/cookie.rs` prints the extracted number and sends the same number in the desktop notification. Authenticator pushes share a three-attempt budget across all refresh sessions in the supervisor. The MFA method picker is debounced so a lingering page cannot be clicked repeatedly.
- If refresh fails or three MFA notifications go unanswered, keeps the supervisor, routing proxy, and PAC server alive without a tunnel. Browser authentication remains paused after the third notification; a later `jumphost authenticate` supplies credentials and a valid cookie, or `jumphost refresh_token` forces a new cookie fetch from already-configured credentials; the monitor then starts the VPN. Staying alive also prevents systemd/launchd failure restarts from resetting the safety limit.
