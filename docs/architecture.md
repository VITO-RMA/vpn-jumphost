...
| **`jumphost` binary** | Single Rust binary built from the repo root (`target/release/jumphost`). One process owns everything: cookie management, openconnect supervision, the routing proxy, and the PAC HTTP server. Subcommands: `run`, `fetch-cookie`, `validate-cookie`, `generate-pac`, `authenticate`, `doctor`. |
