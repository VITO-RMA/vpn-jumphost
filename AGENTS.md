...
- **CLI** — `src/main.rs`. Subcommands: `run` (default), `fetch-cookie`, `validate-cookie`, `generate-pac`, `serve-pac`, `authenticate`. Global flags: `-c / --config FILE` (explicit config file path, overrides XDG default) and `-v / --verbose`. Top-level flags from `run` are accepted at the top level for backwards-compatible UX.
