...
- **CLI** — `src/main.rs`. Subcommands: `run`, `fetch-cookie`, `validate-cookie`, `generate-pac`, `authenticate`, `doctor`, `test-tunnel`. Global flags: `-c / --config FILE` (explicit config file path, overrides XDG default) and `-v / --verbose`.
...
| Keyring / credential storage | `src/credential_store.rs`, `src/config.rs` (credential resolution), `spec.md` (env-var table + credential precedence), `README.md` (Automatic credentials) |
...
| Tunnel probe / `test-tunnel` | `src/probe.rs`, `src/test_tunnel.rs`, `src/config.rs` (defaults + `route_label_for_host`), `src/config_file.rs` (`[probe]` table), `spec.md`, `docs/config.example.toml`, `docs/architecture.md`, `README.md`, `justfile` |
