...
- **CLI** — `src/main.rs`. Subcommands: `run`, `fetch-cookie`, `validate-cookie`, `generate-pac`, `authenticate`, `doctor`. Global flags: `-c / --config FILE` (explicit config file path, overrides XDG default) and `-v / --verbose`.
